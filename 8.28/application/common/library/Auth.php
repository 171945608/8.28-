...
class Auth
{
    ...
    protected $config      = [];
    protected $options     = [];
    protected $allowFields = ['id', 'nickname', 'avatar'];
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            ...
            if (!\app\api\model\User::checkForbid($user_id)) {
    ...
    public function getUserinfo()
    {
        ...
        $extra = [
            'id_auth'  => $data['id_auth'] == 2,
            'vip_auth' => $data['vip_auth'] == 2,
        ];

        $allowFields = $this->getAllowFields();
        $userinfo = array_intersect_key($data, array_flip($allowFields));
        $userinfo = array_merge($userinfo, Token::get($this->_token), $extra);
        return $userinfo;
    }
